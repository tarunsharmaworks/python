...
# time.time()
# The time.time() function returns the current time as a floating-point number, representing the number of seconds since 
# the epoch (the point in time when the time module was initialized). The returned value is based on the computer's system 
# clock and is affected by time adjustments made by the operating system, such as daylight saving time. Here's an example:
...
#  various purposes, such as measuring the duration of an operation or the elapsed time since a certain point in time.
...
# This function is particularly useful for formatting dates and times in a human-readable format, such as 
# for display in a GUI, a log file, or a report. Here's an example:
...
#  as a string, using a specified format. The format string contains codes that represent different parts of the 
# time value, such as the year, the month, the day, the hour, the minute, and the second.

# Conclusion
# The time module in Python provides a set of functions to work with time-related operations, such as timekeeping,
#  formatting, and time conversions. Whether you are writing a script, a library, or an application, the time module is
#  a powerful tool that can help you perform time-related tasks with ease and efficiency. So, if you haven't already, be sure 
# to check out the time module in Python and see how it can help you write better, more efficient code.
import time
...
print(67)
time.sleep(4)
print("this is printed after 4 secs")
# Create the voice dispatcher object
