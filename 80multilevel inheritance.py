...
# Now, when we create an object of the GoldenRetriever class, it has access to all the attributes and methods of the 
# Animal class and the Dog class. We can also see that the GoldenRetriever class has its own attributes and methods that are specific to the class.
...
# As we can see from the output, the GoldenRetriever object has access to all the attributes and methods 
# of the Animal and Dog classes, and, it has also added its own unique attributes and methods. 
...
# In conclusion, multilevel inheritance is a powerful feature in object-oriented programming that allows you to create complex and
#  intricate classes by building upon existing ones. It provides the benefits of code reuse, maintainability, and readability, 
...
class car(veihicle):
    def __init__(self,name,drive):
        veihicle.__init__(self,name,kind="car")
        self.drive=drive
    def show(self):
        veihicle.show(self)
...
class drift_car(car):
    def __init__(self,name,colour):
        car.__init__(self,name,drive="Rear_Wheel")
        self.colour=colour
    def show(self):
        car.show(self)
        print(f"Colour: {self.colour}")  
a=drift_car("AE86","BLack-White")   
a.show()             
print(drift_car.mro())  